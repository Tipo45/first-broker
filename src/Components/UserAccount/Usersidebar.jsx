...
const Usersidebar = ({ activepage }) => {
  ...
  const navigationItems = [
    { to: "/user_account/dashboard", icon: <RxDashboard className="h-6 w-6" />, label: "Dashboard", key: "dashboard", delay: "0.1s" },
    { to: "/user_account/my-trades", icon: <FaExchangeAlt className="h-6 w-6" />, label: "My Trades", key: "my-trades", delay: "0.2s" },
    { to: "/user_account/crypto-accounts", icon: <FaWallet className="h-6 w-6" />, label: "Crypto", key: "crypto-accounts", delay: "0.3s" },
    { to: "/user_account/my-investments", icon: <FaChartLine className="h-6 w-6" />, label: "My Investments", key: "my-investments", delay: "0.4s" },
  ];
    ...
    <section>
      <div className="flex">
        ...
        >
          ...
          <nav className="mt-4">
            <ul>
              {navigationItems.map(({ to, icon, label, key, delay }) => (
                <li key={key}>
                  <Link
                    to={to}
          ...
          {/* User Info & Logout */}
          <div className="mt-auto p-4 border-t border-light-gray animate-zoom-in" style={{ animationDelay: "0.4s" }}>
            <div className="flex items-center">
              <Link to="/user_account/account-information">
                {!isExpanded && <FaUser className="h-8 w-8 hover:text-teal" />}
                {isExpanded && (
                  <div className="ml-0">
            ...
            <button
              ...
              className="mt-4 flex items-center p-2 rounded-lg cursor-pointer hover:bg-red-500 transition-colors duration-200 w-full relative group"
            >
